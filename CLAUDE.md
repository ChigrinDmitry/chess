...
- [docs/ROADMAP.md](docs/ROADMAP.md) — staged plan (0–10) and a **Статус** checklist at the bottom. Stages 0 (scaffold), 1 (design system + themes), 2 (game core: `entities/game`, `clock`, `player`), 3 (pieces + board: `entities/piece`, `widgets/chess-board`, `features/make-move|promote-pawn|flip-board`, `shared/lib/pointer-drag`) and 4 (local game end to end: `/local` with clocks, move list/history, resign, draw offers, result modal; `widgets/game-sidebar|game-controls|game-result-modal|app-header`, `features/navigate-history|resign-game|offer-draw`, `pages/home`) are done; transport, host, bot and session are not yet implemented (`/local` wires the game and clock stores directly in `pages/local-game/model/useLocalGame` until stage 5 moves this into `features/host-game`). Check the roadmap before assuming a module exists (`entities/session|bot`, other `features/` and `widgets/` are not created yet).
...
**Planned architecture (from the spec, not yet built)** — keep new code consistent with it:
...
- All play (including local hot-seat and bot) goes through a `GameTransport` interface in `shared/api` (`LocalTransport`, `BroadcastChannelTransport`, later `WebSocketTransport`), with Zod-validated protocol messages. Authoritative game logic lives in `features/host-game`, not in transports.
...
- Tests are colocated with sources (`*.test.ts(x)`).
